   ...
   // 선택된 국가 처리 함수
    function handleSelection(selectedCountry) {
        ...
        if (coordinates && flagCode) {
            map.flyTo(coordinates, 5.5); // 지도 이동

            //팝업 추가
            const popupContent = `
            <div style="text-align: center; cursor: pointer;" onclick="window.navigateToAnalysis('${selectedCountry}')"> 
